...
public class InsertStudPage extends JFrame {
    ...
    public InsertStudPage() {
        ...
        JLabel lblDataProf = new JLabel("Στοιχεία Μαθητή");
        ...
        btnCloseApp.addActionListener(e -> {
            ...
            Main.getInsertStudPage().setVisible(false);
        });
...
//                // Data Binding
            StudentInsertDTO insertDTO = doDataBinding();
//                // Data Validation
            ...
            // Insert
            try {
                studentReadOnlyDTO = studentService.insertStudent(insertDTO);
                Main.getInsertStudPage().setEnabled(false);
                Main.getInsertStudSuccessPage().setUuidText(studentReadOnlyDTO.getUuid());
                Main.getInsertStudSuccessPage().setVisible(true);
                resetForm();
            } catch (StudentDAOException ex1) {
                JOptionPane.showMessageDialog(null,  "Insertion error", "Error", JOptionPane.ERROR_MESSAGE);
            } catch (StudentAlreadyExistsException ex2) {
...
//
    ...
    private StudentInsertDTO doDataBinding() {
        ...
        return new StudentInsertDTO(firstname, lastname, vat, fatherName, phoneNumber, mail, address, streetNo, cityId, postalCode);
    }
}
