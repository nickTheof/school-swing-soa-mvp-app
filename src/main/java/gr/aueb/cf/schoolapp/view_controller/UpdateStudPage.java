...
public class UpdateStudPage extends JFrame {
    ...
    StudentReadOnlyDTO dto;

    public UpdateStudPage() {
        ...
        JLabel lblDataProf = new JLabel("Στοιχεία Σπουδαστή");
        ...
        addressField.addFocusListener(new FocusAdapter() {
            ...
            public void focusLost(FocusEvent e) {
                ...
                errorAddress.setText(address.isEmpty() ? "Η διέθυνση είναι υποχρεωτικη" : "");
            }
        });
        ...
        btnUpdate.addActionListener(e -> {
            // Data binding
            StudentUpdateDTO updateDTO = doDataBinding();

            // Validation
            Map<String, String> errors = PersonValidator.validate(updateDTO);
            if (!errors.isEmpty()){
            ...
            // Update
            try {
                StudentReadOnlyDTO studentReadOnlyDTO = studentService.updateStudent(dto.getId(), updateDTO);
                    Main.getUpdateStudSuccessPage().setUuidText(studentReadOnlyDTO.getUuid());
                    Main.getUpdateStudPage().setEnabled(false);
                    Main.getUpdateStudSuccessPage().setVisible(true);
        ...
        btnUpdate.setBounds(727, 395, 314, 50);
    ...
    private StudentUpdateDTO doDataBinding() {
        String firstname = nameField.getText().trim();
        String lastname = lastnameField.getText().trim();
        String vat = vatField.getText().trim();
        String phoneNumber = phoneField.getText().trim();
        ...
        return new StudentUpdateDTO(Main.getViewStudsPage().getSelectedId(), firstname, lastname, vat, fatherName, phoneNumber, mail, address, streetNo, cityId, postalCode);
    }
    ...
    private void fetchStudentFromDatabase(Long id) {
        try {
            ...
            nameField.setText(dto.getFirstname());
            vatField.setText(dto.getVat());
            phoneField.setText(dto.getPhoneNum());
            addressField.setText(dto.getStreet());
            lastnameField.setText(dto.getLastname());
            fatherNameField.setText(dto.getFatherName());
            mailField.setText(dto.getEmail());
            streetNoField.setText(dto.getStreetNum());
            postalCodeField.setText(dto.getZipcode());
            int cityId = dto.getCityId();
            City selectedCity = cities.stream().filter(c -> c.getId() == cityId).findFirst().orElse(null);
